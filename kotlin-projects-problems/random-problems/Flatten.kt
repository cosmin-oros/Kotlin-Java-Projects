...
fun main() {
    val listOfListOfAuthors: List<List<Author>> = Library.books.map { it.authors }
//        .also { it.forEach { println(it) } }
    // it prints a list of lists of authors

    val authors: List<Author> = listOfListOfAuthors.flatten()
        .also { it.forEach { println(it) } }
    //it prints a list of authors

    val flatMappedAuthors: List<String> = listOfListOfAuthors.flatMap { authorList ->
    ...
    //print just the name of the authors
}